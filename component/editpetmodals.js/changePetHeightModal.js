...
const ChangePetHeightModal = ({openHeight, onClose, petId}) =>{
    const {updateData} = useCollection("Pets", petId)
    const {data:petData} = useCollection("Pets", petId)
    const petDataRef = useRef()

      const onSave = async () => {
        updateData({height:petDataRef.current.value})
        onClose(false)
      };
    return(
            <Modal open={openHeight} onClose={onClose}>
                <Box sx={style}>
...
export default ChangePetHeightModal;