...
const ChangePetDescriptionModal = ({openPetDescription, onClose, petId}) =>{
    const {updateData} = useCollection("Pets", petId)
    const {data:petData} = useCollection("Pets", petId)
    const petDataRef = useRef()

      const onSave = async () => {
        updateData({description:petDataRef.current.value})
        onClose(false)
      };
    return(
            <Modal open={openPetDescription} onClose={onClose}>
                <Box sx={style}>
                    <Box sx={{display:'flex', flexDirection:"column", alignItems:'center', gap:2}}>
                        ...
                        What's your pet description?
                        </Typography>
                        <Box sx={{margin: ' 10px', display:'flex', alignItems:'center', justifyContent:'center'}}>
                            <TextField placeholder={petData?.description} error={false}
                              name='petDescription'
                              label='Description*'
                              multiline
                              rows={3}
...
export default ChangePetDescriptionModal;