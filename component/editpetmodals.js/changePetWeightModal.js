...
const ChangePetWeightModal = ({openWeight, onClose, petId}) =>{
    const {updateData} = useCollection("Pets", petId)
    const {data:petData} = useCollection("Pets", petId)
    const petDataRef = useRef()

      const onSave = async () => {
        updateData({weight:petDataRef.current.value})
        onClose(false)
      };
    return(
            <Modal open={openWeight} onClose={onClose}>
                <Box sx={style}>
...
export default ChangePetWeightModal;