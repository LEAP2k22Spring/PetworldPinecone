...
const Post = ({ id, userAvatar, createdAt, desc, userName, image, userID }) => {
  ...
  const {
    data: likes,
    deleteData: deleteLike,
    updateData: updateLike,
  } = useSubCollection("Posts", id, "likes");
  const {
    data: comments,
    deleteData: deleteComment,
    createData: createComment,
  } = useSubCollection("Posts", id, "comments");
  const {
    data: follows,
    deleteData: unfollow,
  ...
  const likePost = async () => {
    try {
      if (likes?.find((like) => like?.id === auth?.currentUser?.uid)) {
        ...
        updateLike(auth?.currentUser?.uid, {
  ...
  const followUser = async () => {
    try {
      if (follows?.find((follow) => follow?.id === auth?.currentUser?.uid)) {
        ...
        updateFollow(auth?.currentUser?.uid, {
          userName: userData.firstName,
          userAvatar: userData.avatar,
        });
      }
      ...
      <Card sx={{ width: "350px", borderRadius: "20px" }}>
        ...
        <CardActions sx={{ justifyContent: "space-between" }}>
          <IconButton aria-label="add to favorites" onClick={likePost}>
            <FavoriteIcon
              color={
                likes?.find((like) => like?.id === auth?.currentUser?.uid)
                  ? "error"
                  : "inherit"
              }
            />
          ...
          <Typography sx={{ fontWeight: "400", fontSize: "13px" }}>
            {likes?.length === 0 ? "like" : `liked by ${likes?.length}`}
          </Typography>
          <AvatarGroup
            max={3}
            sx={{
          ...
          {auth?.currentUser?.uid === userID ? (
            ...
            >
              {follows?.find((follow) => follow?.id === auth?.currentUser?.uid)
                ? "Unfollow"
                : "Follow"}
        ...
        {comments?.map((comment, i) => (
          ...
          >
            <Box
              className={styles.comment_history}
              display="flex"
              gap={1}
